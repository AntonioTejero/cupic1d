...
int main (int argc, const char* argv[])
{
  ...
  int num_e, num_i, num_se;             // number of particles (electrons and ions)
  double U_e, U_i, U_se;                // system energy for electrons and ions
  ...
  double vd_e = init_vd_e();            // drift velocity of electrons
  ...
  double *d_avg_ddf_se, *d_avg_vdf_se;      // density and velocity distribution function for electrons
  ...
  int count_phi = 0;                        // |
  int count_E = 0;                          // |
  particle *d_e, *d_i, *d_se;               // particles vectors
  ...
  init_dev();
  init_sim(&d_rho, &d_phi, &d_E, &d_avg_rho, &d_avg_phi, &d_avg_E, &d_e, &num_e, &d_i, &num_i, &d_se, &num_se, 
           &d_avg_ddf_e, &d_avg_vdf_e, &d_avg_ddf_i, &d_avg_vdf_i, &d_avg_ddf_se, &d_avg_vdf_se, &t, &state);
  ...
  for (int i = n_ini+1; i <= n_fin; i++, t += dt) {
    ...
    particle_mover(d_e, num_e, d_i, num_i, d_se, num_se, d_E);
    cc(t, &num_e, &d_e, &dtin_e, &vd_e, &num_i, &d_i, &dtin_i, &vd_i, &num_se, &d_se, &q_p, d_phi, d_E, state);
    ...
    if (i>=n_prev && i%n_save==0) {
      ...
      save_vdf(d_avg_vdf_se, v_max_e, v_min_e, filename);
      ...
      save_log(t, num_e, num_i, num_se, U_e, U_i, U_se, vd_e, vd_i, d_phi);
    }
  }
